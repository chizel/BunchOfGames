...
class Tiar():
    ...
    def new_game(self):
        self.__init__(self.size_r, self.size_c)

    def save_game(self):
        '''Saving game to a file'''
        # TODO save score
        with open(self.PATH + 'save_tiar.txt', 'w') as f:
            json.dump(self.field, f)

    def load_game(self):
        '''Loading saved game'''
        # TODO check is saving exists
        with open(self.PATH + 'save_tiar.txt', 'r') as f:
    ...
    def create_cell(self, row, column):
        ...
        self.ui_field[row][column].bind(
            '<B1-Motion>',
            lambda e: self.move1(e))
        self.ui_field[row][column].bind(
            '<ButtonRelease-1>',
            lambda e: self.move(row, column))
